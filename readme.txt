Ce document est une documentation du projet qui decrit les étapes et une explication rapide des pages et du programme:
    ...
    6. En templates : 
        ...
        base.html.twig : definissez les comportements de page

    7. En test :
        test les donnees qui sont dedier a rentree dans la bdd
        ne pas upload sur github pour les raisons de securite

    8. En var :
        se trouve la bdd locale
    
    9. En .env, .env.local :
        .env : 
            definissez la bdd en phpmyadmin (mysql) => on peut changer le port, CRUD, etc
            definitions de chemin pour le mail de reinitialisation de mot de passe
        .env.local : 
            definissez la bdd en locale (Microsoft access) => on peut que rajouter des lignes dans chaque tableau
            definitions de chemin pour le mail d'assistance
